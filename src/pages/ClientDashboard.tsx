...
export default function ClientDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    if (!effectiveClientId) return;
    const [txData, accountLinks] = await Promise.all([
      fetchAllRows<Transaction>(() =>
        supabase
          .from("transactions")
          .select("id, type, amount, platform, description, date, created_at, status")
    ...
    if (accIds.length > 0) {
      ...
      if (campIds.length > 0) {
        const metrics = await fetchAllRows<any>(() =>
          supabase
            .from("daily_metrics")
            .select("campaign_id, data_date, spend, impressions, clicks, results, conversion_value, synced_at")
