...
export default function PlatformCostAnalytics() {
  ...
  const { data: costs = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("platform_costs").select("*").order("period", { ascending: false });
      return data || [];
    },
  });
  ...
  const { data: orgs = [] } = useQuery({
    queryKey: ["cost-orgs"],
    queryFn: async () => {
      const { data } = await supabase.from("organizations").select("id, status");
      return data || [];
    },
  });
  ...
  const chartData = useMemo(() => {
    ...
    costs.forEach((c) => {
      const m = format(parseISO(c.period), "MMM yy");
      const existing = byMonth.get(m) || { revenue: 0, cost: 0 };
      existing.cost += Number(c.amount_bdt);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button><Plus className="mr-2 h-4 w-4" /> Add Cost</Button>
          </DialogTrigger>
          <DialogContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">৳{Math.round(totalCosts).toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Cost per Tenant</p>
            <p className="text-2xl font-bold">৳{Math.round(costPerTenant).toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <p className={`text-2xl font-bold ${grossMargin >= 50 ? "text-emerald-500" : grossMargin >= 20 ? "text-yellow-500" : "text-destructive"}`}>
