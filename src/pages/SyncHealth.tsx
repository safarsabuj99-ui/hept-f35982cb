...
import {
  Loader2, Zap, RefreshCw, BarChart3, Bell, Target, Activity,
...
export default function SyncHealth() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select("key, value")
      .in("key", SCHEDULE_ROWS.map(r => r.key))
      .then(({ data }: any) => {
        const map: Record<string, string> = {};
        for (const row of data ?? []) map[row.key] = row.value;
        // Fill defaults
        for (const r of SCHEDULE_ROWS) {
          if (!map[r.key]) map[r.key] = r.recommended;
        }
        setSchedules(map);
  ...
  const handleSaveSchedule = async () => {
    ...
    for (const row of SCHEDULE_ROWS) {
      const { error } = await (supabase.from("settings" as any) as any)
        .update({ value: schedules[row.key] || row.recommended, updated_by: user?.id })
        .eq("key", row.key);
      if (error) hasError = true;
    }
    setSavingSchedule(false);
    if (hasError) {
      toast({ title: "Error", description: "Some schedule settings could not be saved.", variant: "destructive" });
    } else {
      toast({ title: "Saved", description: "Sync schedules updated successfully." });
    }
  };
  ...
  const handleForceRetry = async (accountId: string, accountName: string) => {
    ...
    try {
      const { error } = await supabase.functions.invoke("sync-deep-dive", {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Monitor sync status and configure schedules</p>
      </div>
        ...
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Set how often each platform syncs data</CardDescription>
            </div>
          </div>
        </CardHeader>
            ...
            <>
                ...
                {SCHEDULE_ROWS.map(row => {
                  ...
                  return (
                    <div key={row.key} className="flex items-center gap-3 rounded-lg border p-3">
                      ...
                      <Select
                        value={current}
                        onValueChange={(val) => setSchedules(prev => ({ ...prev, [row.key]: val }))}
                      >
              ...
              <div className="flex items-start gap-2 rounded-lg bg-muted/50 p-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Recommendations are based on each platform's API rate limits and data reporting lag. 
                  TikTok has a 15-30 min data delay and strict rate limits, so syncing more frequently wastes API calls.
                </p>
              </div>
