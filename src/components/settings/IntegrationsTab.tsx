...
export function IntegrationsTab() {
  ...
  const saveIntegration = async (id: string, updates: any) => {
    ...
    else { toast({ title: "Saved" }); fetchData(); }
  };
  ...
  const addInstance = async () => {
    ...
    const { error } = await (supabase.from("api_integrations" as any) as any).insert({
      platform: newForm.platform, instance_name: newForm.instance_name, api_token: newForm.api_token,
      app_id: newForm.app_id, token_expiry_date: newForm.token_expiry_date || null, updated_by: user?.id, is_active: true,
    });
    setSaving(false);
    if (error) toast({ title: "Error", description: error.message, variant: "destructive" });
    else {
  ...
  const testConnection = async (id: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("test-connection", { body: { integration_id: id } });
      if (error) throw error;
      setTestResults((prev) => ({ ...prev, [id]: data }));
      toast({ title: data.ok ? "Connection Successful" : "Connection Failed", description: data.message, variant: data.ok ? "default" : "destructive" });
    } catch (err: any) {
  ...
  const simulateSync = async () => {
    ...
    try {
      ...
      toast({ title: "Sync Complete", description: `Synced ${res.data.synced || 0} accounts` });
      fetchData();
    } catch (err: any) {
  ...
  const getTokenHealth = (inst: any): { status: "ok" | "warning" | "expired" | "unknown"; label: string } => {
    if (!inst.token_expiry_date) return { status: "unknown", label: "No expiry set" };
    const days = differenceInDays(new Date(inst.token_expiry_date), new Date());
    if (days <= 0) return { status: "expired", label: "Expired" };
    if (days <= 3) return { status: "expired", label: `${days}d left` };
  ...
  const grouped = PLATFORMS.map((p) => ({ ...p, instances: integrations.filter((i: any) => i.platform === p.key) }));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-muted-foreground text-sm">Configure multi-instance platform API credentials</p>
        </div>
              ...
              <div className="space-y-4">
                ...
                {SETUP_GUIDES[newForm.platform] && (
                  <div className="rounded-lg border bg-muted/50 p-4 space-y-3">
                    ...
                    <ol className="space-y-1.5 text-xs text-muted-foreground list-decimal list-inside">
                      {SETUP_GUIDES[newForm.platform].steps.map((step, i) => <li key={i} className="leading-relaxed">{step}</li>)}
                    </ol>
                    <a href={SETUP_GUIDES[newForm.platform].link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
                      <ExternalLink className="h-3 w-3" /> {SETUP_GUIDES[newForm.platform].linkLabel}
                    </a>
                  </div>
                )}
                <div className="space-y-2">
                ...
                {newForm.platform === "tiktok" && (
                  <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 space-y-3">
                    <div className="flex items-center gap-2 text-sm font-medium"><Zap className="h-4 w-4 text-primary" /> TikTok OAuth — Exchange Auth Code for Token</div>
                    <p className="text-xs text-muted-foreground">After authorizing your app, paste the auth_code from the redirect URL here.</p>
                    <div className="space-y-2">
                      <Label className="text-xs">TikTok Developer App ID</Label>
                      <Input value={tiktokAppId} onChange={(e) => setTiktokAppId(e.target.value)} placeholder="e.g. 7615506076583673857" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs">App Secret</Label>
                      <Input type="password" value={tiktokAppSecret} onChange={(e) => setTiktokAppSecret(e.target.value)} placeholder="From Developer Portal → Basic Information" />
                    </div>
                    <div className="space-y-2">
                      <Label className="text-xs">Auth Code</Label>
                      <Input value={tiktokAuthCode} onChange={(e) => setTiktokAuthCode(e.target.value)} placeholder="Paste auth_code from redirect URL" />
                    </div>
                    <Button variant="outline" size="sm" className="w-full gap-2" disabled={exchangingToken || !tiktokAuthCode.trim() || !tiktokAppId.trim() || !tiktokAppSecret.trim()}
                      onClick={async () => {
                        setExchangingToken(true); setExchangeResult(null);
                        try {
                          const { data, error } = await supabase.functions.invoke("tiktok-exchange-token", { body: { auth_code: tiktokAuthCode.trim(), app_id: tiktokAppId.trim(), app_secret: tiktokAppSecret.trim() } });
                          if (error) throw error;
                          if (data.ok) { setNewForm((prev) => ({ ...prev, api_token: data.access_token, token_expiry_date: data.expiry_date || "" })); setExchangeResult({ ok: true, message: data.message }); toast({ title: "Token Obtained!", description: data.message }); }
                          else { setExchangeResult({ ok: false, message: data.message }); toast({ title: "Exchange Failed", description: data.message, variant: "destructive" }); }
                        } catch (err: any) { setExchangeResult({ ok: false, message: err.message }); toast({ title: "Exchange Failed", description: err.message, variant: "destructive" }); }
                    ...
                    {exchangeResult && (
                      <div className={`rounded-md p-2.5 text-xs ${exchangeResult.ok ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400" : "bg-destructive/10 border border-destructive/30 text-destructive"}`}>
                        ...
                        {exchangeResult.ok && <p className="mt-1 text-muted-foreground">✅ Token & expiry auto-filled below</p>}
                      </div>
                    )}
                  </div>
                )}
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">Leave blank for tokens that never expire</p>
                </div>
      ...
      {totalIntegrations > 0 && (
        <div className="grid gap-4 sm:grid-cols-4">
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><Plug className="h-5 w-5 text-muted-foreground" /><div><p className="text-2xl font-semibold">{totalIntegrations}</p><p className="text-xs text-muted-foreground">Total</p></div></div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-emerald-500" /><div><p className="text-2xl font-semibold">{healthyCount}</p><p className="text-xs text-muted-foreground">Healthy</p></div></div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><AlertTriangle className="h-5 w-5 text-yellow-500" /><div><p className="text-2xl font-semibold">{warningCount}</p><p className="text-xs text-muted-foreground">Expiring Soon</p></div></div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3"><XCircle className="h-5 w-5 text-destructive" /><div><p className="text-2xl font-semibold">{expiredCount}</p><p className="text-xs text-muted-foreground">Expired</p></div></div></CardContent></Card>
        </div>
      )}
        ...
        <TabsContent value="connections" className="space-y-6 mt-4">
          {grouped.map(({ key, label, instances }) => (
            <div key={key} className="space-y-3">
              ...
              {instances.length === 0 ? (
                <Card className="border-dashed"><CardContent className="py-6 text-center text-muted-foreground text-sm">No instances configured. Click "Add Instance" to connect.</CardContent></Card>
              ) : (
                    ...
                    return (
                      <Card key={inst.id} className={health.status === "expired" ? "border-destructive/50" : ""}>
                        <CardHeader className="pb-2">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-sm font-medium">{inst.instance_name || key}</CardTitle>
                            <div className="flex items-center gap-1.5">
                              {health.status === "expired" && <Badge variant="destructive" className="text-[10px]">{health.label}</Badge>}
                              {health.status === "warning" && <Badge className="bg-yellow-500/10 text-yellow-600 border-yellow-500/30 text-[10px]"><AlertTriangle className="h-2.5 w-2.5 mr-0.5" /> {health.label}</Badge>}
                              {health.status === "ok" && <Badge variant="outline" className="text-[10px] text-emerald-600"><Shield className="h-2.5 w-2.5 mr-0.5" /> {health.label}</Badge>}
                              {health.status === "unknown" && <Badge variant="outline" className="text-[10px]">No expiry</Badge>}
                              <Badge variant="default" className={`text-[10px] ${inst.connection_status === "error" ? "bg-destructive text-destructive-foreground" : "bg-emerald-500/10 text-emerald-600 border-emerald-500/30"}`}>
                          ...
                          {inst.last_synced_at && <CardDescription className="flex items-center gap-1 text-xs"><Clock className="h-3 w-3" /> Synced: {new Date(inst.last_synced_at).toLocaleString()}</CardDescription>}
                        </CardHeader>
                        <CardContent className="space-y-3">
                          <div className="space-y-1"><Label className="text-xs">API Token</Label><Input type="password" defaultValue={inst.api_token} onBlur={(e) => { if (e.target.value !== inst.api_token) saveIntegration(inst.id, { api_token: e.target.value }); }} /></div>
                          <div className="space-y-1"><Label className="text-xs">App ID</Label><Input defaultValue={inst.app_id} onBlur={(e) => { if (e.target.value !== inst.app_id) saveIntegration(inst.id, { app_id: e.target.value }); }} /></div>
                          <div className="space-y-1"><Label className="text-xs">Token Expiry</Label><Input type="date" defaultValue={inst.token_expiry_date || ""} onBlur={(e) => { const newVal = e.target.value || null; if (newVal !== inst.token_expiry_date) saveIntegration(inst.id, { token_expiry_date: newVal }); }} /></div>
                          <Button variant="outline" size="sm" className="w-full gap-2" disabled={isTesting} onClick={() => testConnection(inst.id)}>
                          ...
                          {testResult && (
                            <div className={`rounded-md p-2.5 text-xs space-y-1 ${testResult.ok ? "bg-emerald-500/10 border border-emerald-500/30" : "bg-destructive/10 border border-destructive/30"}`}>
                              ...
                              {testResult.details && <p className="text-muted-foreground pl-5">{testResult.details}</p>}
                            </div>
                          )}
                        </CardContent>
                      </Card>
          ...
          {PLATFORMS.map((p) => {
            ...
            return (
              <Card key={p.key}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-base flex items-center gap-2"><BookOpen className="h-4 w-4 text-primary" /> {p.label} Setup Guide</CardTitle>
                    <a href={guide.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline"><ExternalLink className="h-3 w-3" /> {guide.linkLabel}</a>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <h4 className="text-sm font-medium mb-2">Step-by-Step Instructions</h4>
                    <ol className="space-y-2 text-sm text-muted-foreground list-decimal list-inside">{guide.steps.map((step, i) => <li key={i} className="leading-relaxed">{step}</li>)}</ol>
                  </div>
                  <div className="rounded-lg bg-muted/50 p-4">
                    <h4 className="text-sm font-medium mb-2 flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-primary" /> Pro Tips</h4>
                    <ul className="space-y-1.5 text-xs text-muted-foreground">{guide.tips.map((tip, i) => <li key={i} className="flex items-start gap-2"><CheckCircle2 className="h-3 w-3 text-emerald-500 mt-0.5 shrink-0" />{tip}</li>)}</ul>
                  </div>
                </CardContent>
              </Card>
            );
