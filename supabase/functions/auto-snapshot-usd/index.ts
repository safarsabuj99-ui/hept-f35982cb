...
async function snapshotForOrg(supabase: any, orgId: string, today: string) {
  // 1. Latest manual baseline FOR THIS ORG
  ...
  // 2. Sums since baseline — ALL org-scoped
  const purchaseFilter = (q: any) => {
    let qq = q.eq("org_id", orgId);
    if (baselineDate) qq = qq.gte("date", baselineDate);
    return qq;
  };
  const spendFilter = (q: any) => {
    let qq = q.eq("org_id", orgId);
    if (baselineDate) qq = qq.gte("data_date", baselineDate);
    return qq;
  };
  const manualFilter = (q: any) => {
    let qq = q.eq("org_id", orgId);
    if (baselineDate) qq = qq.gte("date", baselineDate);
    return qq;
  };
  ...
  const burn7 = await fetchAll(
    ...
    (q: any) => q.eq("org_id", orgId).gte("data_date", sevenDaysAgoStr),
  );
  ...
  // 4. Client obligations — ORG-SCOPED transactions (THE CRITICAL FIX)
  const txns = await fetchAll(
    supabase, "transactions", "type, amount, client_id",
    (q: any) => q.eq("org_id", orgId).eq("status", "completed"),
  );
  ...
  // 5. Client names for breakdown (org-scoped)
  ...
  // 6. Check if today already has a manual baseline FOR THIS ORG
  ...
  if (isManualToday) {
    // Update derived fields only — never touch baseline_balance_usd or notes
    ...
    const { error: upsertErr } = await supabase
      ...
      .upsert(
        ...
        { onConflict: "snapshot_date,org_id" },
      );
    if (upsertErr) throw upsertErr;
  }

  console.log(
    `[org ${orgId}] $${balance.toFixed(2)} on ${today} | obligations=$${clientObligations.toFixed(2)} (${positiveClientIds.length} clients) | baseline=${baselineDate ?? "none"}`,
  );

  return { org_id: orgId, balance: r2(balance), client_obligations: r2(clientObligations) };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch ALL organizations and snapshot each one independently
    ...
    for (const orgId of orgIds) {
      try {
        const r = await snapshotForOrg(supabase, orgId, today);
        results.push(r);
      } catch (e) {
        console.error(`Snapshot failed for org ${orgId}:`, e);
