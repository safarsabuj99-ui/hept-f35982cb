...
BEGIN
  ...
  -- Client balances WITH per-platform breakdown
  ...
  client_txns AS (
    ...
    GROUP BY cp.user_id, cp.full_name, cp.email, cp.business_name, cp.pricing_config, t.platform
  ),
  client_balances AS (
    SELECT
      ct.user_id,
      ...
      MAX(ct.business_name) AS business_name,
      MAX(ct.pricing_config::text)::jsonb AS pricing_config,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'platform_balances', cb.platform_balances
  )), '[]'::jsonb)
  INTO v_clients
