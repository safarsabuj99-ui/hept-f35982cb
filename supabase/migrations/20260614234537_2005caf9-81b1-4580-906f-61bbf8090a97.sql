...
BEGIN
  -- Range spend: raw per-campaign platform spend (same source as Campaign tab).
  ...
  -- Yesterday spend: same source, for the comparison delta.
  SELECT COALESCE(SUM(dm.spend), 0)
  ...
  -- 7-day spend sparkline from daily_metrics.
  ...
  FROM (
    ...
    LEFT JOIN campaigns c ON c.id = dm.campaign_id AND c.org_id = v_org_id
    WHERE dm.id IS NULL OR c.id IS NOT NULL
    GROUP BY d
  ) sub;
