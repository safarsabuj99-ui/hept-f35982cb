...
BEGIN
  ...
  -- Range spend: read from daily_ad_spend (Dhaka-calendar dated) so the
  -- "Today" KPI doesn't show $0 between Dhaka midnight and ~6 AM while
  -- platform APIs still report yesterday's date.
  SELECT COALESCE(SUM(final_billable_usd), 0)
  INTO v_range_spend
  FROM daily_ad_spend
  WHERE org_id = v_org_id
    AND (v_mapped_account_ids IS NULL OR ad_account_id = ANY(v_mapped_account_ids))
    AND date >= p_date_from
    AND date <= p_date_to;

  -- Yesterday spend: same source for consistent comparison.
  SELECT COALESCE(SUM(final_billable_usd), 0)
  ...
  -- 7-day spend sparkline: also from daily_ad_spend for consistency.
  SELECT COALESCE(jsonb_agg(daily_val ORDER BY d), '[]'::jsonb)
  INTO v_spend_history
  FROM (
    SELECT d, COALESCE(SUM(das.final_billable_usd), 0) AS daily_val
    FROM generate_series(v_seven_ago, (CURRENT_DATE AT TIME ZONE 'Asia/Dhaka')::date, '1 day'::interval) AS d
    LEFT JOIN daily_ad_spend das ON das.date = d::date
      AND das.org_id = v_org_id
      AND (v_mapped_account_ids IS NULL OR das.ad_account_id = ANY(v_mapped_account_ids))
    GROUP BY d
  ) sub;
  ...
  client_profiles AS (
    SELECT
      ...
      p.is_active,
      p.guard_paused_at,
      p.system_paused_campaigns
    FROM profiles p
    JOIN client_users cu ON cu.user_id = p.user_id
  ),
  client_mappings AS (
    SELECT
      aac.client_id AS user_id,
