...
DECLARE
  ...
  v_totals jsonb;
BEGIN
  -- WAC with cascading fallback (range -> current month -> all-time)
  ...
  -- Aggregate spend per (ad_account, client, platform) restricted to active campaigns
  WITH active_camps AS (
    SELECT c.id, c.ad_account_id, c.client_id, c.platform, c.status
    FROM campaigns c
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(aa.account_name, aa.account_id_external, aa.id::text) AS account_name,
      ag.platform,
      ag.client_id,
      ...
      -- rate lookup mirroring getPlatformRates: flat_rates -> platform_rates -> 120
      COALESCE(
