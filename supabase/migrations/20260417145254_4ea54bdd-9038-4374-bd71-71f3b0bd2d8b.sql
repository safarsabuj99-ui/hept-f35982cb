-- Fix 1: Rewrite mark_parent_complete to SUM rows across all sibling chunks
-- and only finalize stats when all siblings are done. Adaptive chunking
-- thresholds inlined so total_rows_last_sync drives the next-run chunk size.
...
BEGIN
  ...
  -- Standalone (non-chunked) job: stats use the single job's rows directly.
  IF v_parent IS NULL THEN
    ...
    SET avg_rows_per_day = EXCLUDED.avg_rows_per_day,
        ...
        consecutive_failures = CASE
          WHEN v_self_status = 'failed'
            THEN sync_account_stats.consecutive_failures + 1
          ELSE 0
        END,
  ...
  -- Chunked job: aggregate across all siblings under the same parent.
  SELECT
    COUNT(*),
  ...
  -- Adaptive chunk size based on real measured workload across all siblings.
  v_recommended := CASE
    WHEN v_total_rows > 800 THEN 3
    WHEN v_total_rows > 400 THEN 5
    WHEN v_total_rows > 150 THEN 10
