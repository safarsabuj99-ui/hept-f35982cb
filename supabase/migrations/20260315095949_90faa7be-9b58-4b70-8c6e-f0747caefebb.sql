...
BEGIN
  ...
  IF v_guard_paused_at IS NOT NULL AND 
    ...
    -- Window expired: clear system_paused_campaigns (UI shows Active) but do NOT resume campaigns
  ...
  -- Within window: if balance is above double the threshold, auto-resume
  IF v_balance > (COALESCE(v_pause_threshold, 5) * 2) THEN
    ...
    -- Clear system paused campaigns and guard_paused_at
    UPDATE public.profiles
    SET system_paused_campaigns = '[]'::jsonb,
        guard_paused_at = NULL
    WHERE user_id = v_client_id;
